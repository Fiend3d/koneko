...
func main() {
	...
	if flag.NArg() < 1 {
		fmt.Fprintf(os.Stderr, "Usage: koneko [-tab-width=N] [-no-line-numbers] [-no-scrollbar] <file>\n")
		os.Exit(1)
	}
	filePath := flag.Arg(0)

	p := tea.NewProgram(initialModel(filePath, *tabWidth, !*noLineNumbers, !*noScrollbar))
